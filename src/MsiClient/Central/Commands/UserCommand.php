...
class UserCommand extends Command
{
    ...
    {
        return $this->perform([], \MsiClient\Client::GET_REQUEST, $this->getUrl() . "/$userId/token/$token");
    }

    public function setRememberToken($identifier, $token)
    {
        return $this->perform(['token' => $token], \MsiClient\Client::PUT_REQUEST, $this->getUrl() . "/$identifier/remember-token");
    }
    ...
    public function getByCredentials($email, $password)
    {
        return $this->perform(
            [
                'email' => $email,
                'password' => $password,
            ], \MsiClient\Client::POST_REQUEST, $this->getUrl() . "/by-credentials");
    }
}